...
| `POST /api/factories/{id}/activity/csv`, `POST /api/factories/{id}/leak-assessments/csv` | bulk versions of the above and of the leak-assessment endpoints — one CSV row per (process, fuel, month) reading or per leak assessment, straight from the same fields already on a utility bill/compressor log/refrigerant invoice. Templates at `GET /api/csv-templates/activity` and `GET /api/csv-templates/leak-assessments`. |
